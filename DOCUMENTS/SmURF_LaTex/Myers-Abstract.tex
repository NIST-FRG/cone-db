...
\addbibresource{../Cone_References.bib}
% For email linking
\usepackage{hyperref}

\title{ENFP429 NIST (National Institute of Standards and Technology) Abstract}
...
\maketitle
\section{Abstract}
Avery material that people interact with in their daily lives burns a little bit differently. When choosing a material for use in any project, it can be important to understand what it takes for that material to begin burning, and how it will act once it does ignite, including things like how fast the fire will spread and how likely it is that the fire will spread at all. 

One method of collecting this crucial data is the cone calorimeter. In this measurement tool, a piece of the material is placed under a cone shaped heating implement, in order to ensure that the material is heated evenly from above. The heating implement emits a known quantity of energy, and the time until the material ignites (t\_ign) is measured. Sometimes t\_ign measures pilot ignition, which is ignition with an additional hot spot held near the material within the cone. Once the material has ignited, rate at which heat is being released by the material, or heat release rate (HRR), is measured, usually through oxygen consumption. By measuring the concentration of oxygen in the air that is flowing into the calorimeter, and the concentration in the air that is flowing out, we can determine how many grams oxygen were consumed. Since oxygen consumption has a relatively constant heat of combustion, this allows us to determine the heat released from the burning material. Measurement of the air make up before and after exposure to the flame also allows us to take measurements of additional products like CO and CO2, which serve to indicate combustion efficiency. Heat of pyrolysis and heat of combustion, the energy required to break down a material and the energy released from its combustion with oxygyen, respectively, are often measured. Other measurements such as mass yield are taken as well.

Government agencies like NIST or the National Bureau of Standards have been performing these tests for decades, storing the results on many materials in boxes in offices. The overarching goal of this project is to take data from the cone calorimetry tests performed by NIST/the NBS over the past 40 years, currently stored in a physical form, and create a digital, searchable database. The intent is that if someone has a material that they wish to know the data for, they can easily find tests performed on that material in the past, along with the relevant test conditions and results.

 The first step of this process is digitization. The physical data sheets are scanned, and then AI tools are used to take the data from the scanned image and create a digital data file. This file is then parsed into two smaller files. One is referred to as the data file, which contains the data resulting from the tests, like mass and energy emission, and the other is the metadata file, which contains the details of the test, like what energy input level was used, what the starting mass and time were, and the orientation of the material. The next step is to check these data and metadata files for mistakes. The process of checking the data and metadata files against the original scanned documents and editing them to remove transcription errors is known as Systematic User Review of Files, or “SmURFing.” An additional part of SmURFing is creating a Material ID, a name for the files that contains the material and primary testing conditions so that anyone can easily find the test that best matches what they are looking for.
 ...
 The two measurements that were prioritized most in our SmURFing of this report  were t\_ign until pilot ignition and HRR at time intervals during the burn. This report also included a thermal sensitivity index, a measure of flammability derived from heat of combustion and heat of pyroloysis. There was additionally an external sensitivity index made to indicate if a flame were self sustaining in the absence of the heat input from the calorimeter.
  
 The other report that I focused on was ADA168251 \cite{ADA168251}. The goal of this report was to analyze various composite panels for their practicality of use as aircraft panel materials. The composites tested within the report were all of similar construction, being made of a woven fiber of fiberglass, Kevlar, or graphite and impregnated with either epoxy or phenolic resin and cured into a 1/8" honeycomb. They also had facings of the material making up their fibers, and a 2 mm white Tedlar covering on one side. 
 
 Similarly to the previous report, the most important data to us from this report were the ignition time (t\_ign) until pilot ignition and Heat Release Rate. This procedure does explicitly have edge protection for the horizontal tests, as edge burning is not a concern in the panel's actual use case. 
