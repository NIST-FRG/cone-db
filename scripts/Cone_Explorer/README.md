...
##### Overview
This is the page where SmURFing will be perfomed. Upon opening the explorer, click "Import Data to Explorer" to pull in local copies of the data files. The import button will look at the Parsed metadata folder, pulling new or newly SmURFed data (as obtained from the GitHub repository). Any files labled as bad (i.e. calibration checks, no data, etc...) will be removed from the explorer if they are present. In the case that files have been modified and saved locally in the explorer, but not yet exported to the repo, an error will be raised asking the user to export or discard these changes. 

Type the name of the test you would like to SmURF into the dropdown menu to select it. By Default, only tests that have not undergone SmURFing can be selected; however, the Include SmURFed Tests checkbox in the sidebar allows these tests to be viewed aswell. Note that when SmURFed tests are included, they will be found under their original test name.
The sidebar also includes a checkbox to show only tests that have been queued for SmURFing. Tests are queued in the Metadata Search page (see that page's documentation for more details). Using the queue is recommended when SmURFing by report (i.e, use Metadata Search to queue all tests associated with that report and only SmURF from that queue).
...
MAKE SURE Heat Flux, Material ID, Orientation, and Specimen Number are populated prior to export. Ignition source, Edge Frame, Grid, and Thickness should be populated, or you will get a warning prior to export. These optional parameters should be included wherever they exist.                                                                                                                                  
...
##### Queue for SmURFing
This page should be used when looking for tests to connect to a report. Look up relevant material names, conditions, or other indentifiers from the report. Relevant tests will populate the table. Click the "Add to Queue" checkbox to add desired tests to the queue to be SmURFed. Click `Select All Displayed` to add all tests currently in the table to the queue. Click `Clear Entire Queue` to remove all entries. When at least one test is in the queue, a drop down menu will appear showing which tests are there, and whether they are shown or hidden in the current table of tests. Tests can also be removed individually in this menu. To making Queueing tests simpler, toggle the `View Critical Metadata Only` switch to reduce the number of columns to look through. The table will display File name, Add to Queue, Material Name, Test Date, Heat Flux (kW/m2), Orientation, and Comments. The search function will still look through all fields.                                                                                                                                                                               
