...
try:
    if len(test_selection) != 0:
        ...
        for i, test_stem in enumerate(test_selection):
            if "Average" in test_stem:
                flux = int(test_stem.split('_')[1].replace('kW', ''))
                ...
                for column in [
                                    ...
                                    'Mass Loss (g)', "CO2 Production (g/s)", 'CO2 (kg/kg)',
                                    "CO Production (g/s)", "O2 Consumption (g/s)", "H2O (kg/kg)",
                                    "HCl (kg/kg)", "H'carbs (kg/kg)", "Soot Production (g/s)",
    ...
    ########################################### Generate Plots ###################################################################################      
        ...
        if x_axis_column and y_axis_columns:
            for y_column in y_axis_columns:
                fig = go.Figure()
                errorbar_HRRPUA = False
                missing_data_messages = []
                traces = 0

                if "HRRPUA (kW/m2)" == y_column and any(
                    "Average" in name for name in test_selection
                ):
                    errorbars1 = st.checkbox(
                        "View Uncertainty in Average Heat Release Rate per Unit Area", value=False
                    )
                for i in range(len(test_data)):
                    ...
                    if not y_data.isnull().all() and not x_data.isnull().all():
                        if "Average" in test_selection[i] and y_column == "HRRPUA (kW/m2)":
                            fig.add_trace(
                                go.Scatter(
                                    x=x_data,
                                    y=y_data,
                                    ...
                                    error_y=dict(
                                        ...
                                        visible=True if errorbars1 else False,
                                    ),
                                )
                            )
                        else:
